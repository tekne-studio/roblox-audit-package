...
local function ensureRokitTools()
	...
	local tools = {
		...
		{ name = "selene", spec = "Kampfkarren/selene@0.29.0" },
...
luau = true
		...
		if writeFile("selene.toml", seleneConfig) then
			print("📝 Created selene.toml (auto-initialized)")
			created = true
		end
	end
...
enabled = false
		...
		if writeFile("stylua.toml", styluaConfig) then
			print("📝 Created stylua.toml (auto-initialized)")
			created = true
		end
	end
...
if commandExists("tree") then
	...
	print("   ✓ audit/audit-tree.txt")
else
	print("   ⏭️  Skipped (tree not installed)")
	if IS_WINDOWS then
		print("      💡 choco install tree")
	else
		print("      💡 brew install tree")
	end
end
...
-- 2. Rojo Sourcemap
-- -------------------------------------------
print("🗺️  Generating Rojo sourcemap...")
if commandExists("rojo") then
	...
	if handle then
		...
		if projectFile and projectFile ~= "" then
			...
			print("   ✓ sourcemap.json (from " .. projectFile .. ")")
		else
			print("   ⚠️  No .project.json found")
		end
	end
else
	print("   ⏭️  Skipped (rojo not installed)")
	print("      💡 rokit install")
end
...
-- -------------------------------------------
print("🔍 Running luau-lsp analyze...")
if commandExists("luau-lsp") then
	...
	if grepExists("audit/audit-check.txt", "error:") or grepExists("audit/audit-check.txt", "TypeError:") then
		...
		print("   ⚠️  audit/audit-check.txt (" .. errorCount .. " type errors found)")
	else
		print("   ✓ audit/audit-check.txt (no errors 🎉)")
	end
else
	print("   ⏭️  Skipped (luau-lsp not installed)")
	print("      💡 rokit install")
end
...
if commandExists("stylua") then
	...
	if result == 0 or result == true then
		print("   ✓ audit/audit-style.txt (formatted correctly 🎉)")
	else
		if grepExists("audit/audit-style.txt", "error parsing") then
			local parseErrors = grepCount("audit/audit-style.txt", "error parsing")
			print("   ⚠️  audit/audit-style.txt (" .. parseErrors .. " parse errors)")
			print("      💡 StyLua doesn't support single-line 'continue' yet")
		else
			local diffCount = grepCount("audit/audit-style.txt", "^Diff in")
			if diffCount > 0 then
				print("   ⚠️  audit/audit-style.txt (" .. diffCount .. " files need formatting)")
			else
				print("   ⚠️  audit/audit-style.txt (has issues)")
			end
		end
	end
else
	print("   ⏭️  Skipped (stylua not installed)")
	print("      💡 rokit install")
end
...
if dirExists("sync") then
	...
	if dirExists("src") then
		...
		structureMsg = string.format(
			[[⚠️  Both 'src/' and 'sync/' directories found
...
Recommendation:
  ...
  - Remove sync/ after migration]],
		...
		structureMsg = string.format(
			[[⚠️  Using 'sync/' directory (%d files)
...
Recommendation:
  ...
  - Update default.project.json to reference src/]],
	...
	print("   ⚠️  audit/audit-structure.txt (sync/ is obsolete, migrate to src/)")
elseif dirExists("src") then
	print("   ✓ Project structure looks good (using src/)")
else
...
print(string.format("   📊 .lua: %d files, .luau: %d files", luaCount, luauCount))
if luaCount > 0 then
	print("   💡 Consider migrating .lua → .luau for better LSP support")
end
...
if commandExists("dot") then
	...
	if success1 then
		...
		if dotFile then
			dotFile:close()
			print("   ✓ audit/audit-graph.dot (source)")
		end
	else
		print("   ⚠️  Failed to generate grouped graph")
	end
	...
	if success3 then
		...
		if fullDotFile then
			fullDotFile:close()
			print("   ✓ audit/audit-graph-full.dot (source)")
		end
	else
...
if io.open("audit/audit-selene.txt", "r") then
	...
	if seleneWarnings > 0 then
		print("   🔎 Selene warnings: " .. seleneWarnings)
...
if totalIssues == 0 then
	...
	print("⚠️  Total issues to review: " .. totalIssues)
end

print("")
print("💡 To auto-format code: stylua " .. SRC_DIR .. "/")
print("")
